...
console.log(kzg)  
kzg.default().then(module => {

    const loadTrustedSetup = module.cwrap('load_trusted_setup_file_from_wasm', null,[] )
    const loadTrustedSetupFromInputs = module.cwrap('load_trusted_setup_from_inputs', 'number',['array', 'number', 'array', 'number'] )
   ...
   // loadTrustedSetup()
   // freeTrustedSetup()
    const json = JSON.parse(fs.readFileSync('trusted_setup.json', { encoding: 'utf-8'}))
    console.log(bytesFromHex(json.g1))
    const ret = loadTrustedSetupFromInputs(bytesFromHex(json.g1), 4096, bytesFromHex(json.g2), 95)
    console.log(ret)
